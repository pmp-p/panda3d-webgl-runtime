...
import panda3d.core
panda3d.core.getModelPath().appendDirectory('rsr/asteroids')
...
def genLabelText(text, i):
    return OnscreenText(text=text, parent=base.a2dTopLeft, pos=(0.07, -.06 * i - 0.1),
                        fg=(1, 1, 1, 1), align=TextNode.ALeft,  scale=.05) #shadow=(0, 0, 0, 0.5),
...
class AsteroidsDemo(ShowBase):
    ...
    def __init__(self):
        ...
        self.bg = loadObject("stars.txo", scale=146, depth=200,
                             transparency=False)

        # Load the ship and set its initial velocity.
        self.ship = loadObject("ship.txo")
    ...
    def spawnAsteroids(self):
        ...
        for i in range(10):
            ...
            # from "asteroid1.txo" to "asteroid3.txo".
            asteroid = loadObject("asteroid%d.txo" % (randint(1, 3)),
    ...
    def fire(self, time):
        ...
        bullet = loadObject("bullet.txo", scale=.2)  # Create the object
        bullet.setPos(pos)
